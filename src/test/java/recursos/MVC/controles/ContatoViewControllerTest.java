...
class ContatoViewControllerTest {
    Persistencia db;
    ...
    void remover() {
        Persistencia.droparTodasAsTabelas();
        Persistencia db1 = new Persistencia(true);
		
		//Groups
		Tabela grupo = db1.construirTabela(new Grupo());
		db1.executar( grupo.toSQLCreate() );
		
		//Contacts
		Tabela contato = db1.construirTabela(new Contato());
		db1.executar( contato.toSQLCreate() );
		
		//Phones
		Tabela telefone = db1.construirTabela(new Telefone());
		db1.executar( telefone.toSQLCreate() );
		Tabela[] tbs = { contato, grupo };
		...
		for (Tabela t : tbs) {
		    ...
		    Coluna c5 = new Coluna(nomeColuna,"", constraint);
		    colunas.add(c5);
		...
		db1.executar(tabelaRelacionamento.toSQLCreate());
		Tabela[] tbs1 = { contato, telefone };
		StringJoiner nomeTabela1 = new StringJoiner("_");
		ArrayList<Coluna> colunas1 = new ArrayList<>();
		
		for (Tabela t : tbs1) {
		    String nomeColuna = t.getNome().concat("_fk");
		    String constraint = "REFERENCES " + t.getNome();
		
		    Coluna c4 = new Coluna(nomeColuna,"", constraint);
		    colunas1.add(c4);
		
		    nomeTabela1.add(t.getNome());
		}
		
		Tabela tabelaRelacionamento1 = new Tabela(nomeTabela1.toString(), colunas1);
		
		db1.executar(tabelaRelacionamento1.toSQLCreate());
        this.db = new Persistencia();
